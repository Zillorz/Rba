...
fn number() -> Parser<u8, u64> {
    ...
    integer
        ...
        .convert(|s| u64::from_str(&s.replace("_", "")))
}

fn val() -> Parser<u8, Val> {
    var().map(Val::Var)
        | number().map(|w| Val::Const(Const::Word(w)))
        | string().map(|x| {
            let ret = Val::Const(Const::Str(x.as_ptr()));
            std::mem::forget(x); // We leak all the strings...
            ret
        })
}
...
pub fn fcall() -> Parser<u8, (String, Vec<Val>, Option<Var>)> {
    (label() + space() * list(val(), sym(b',') * space()) - space() + var().opt())
        .map(|((a, b), c)| (a, b, c))
}
...
// We allow this here as the OR is obvious to a parser.
